...
function sortNodes(list) {
  list.sort((a, b) => {
    ...
    // 找到最近共同祖先
    let i = 0;
    while (aList[i] === bList[i]) i += 1;

    // 检查顺序
    const { children } = aList[i - 1];
...
class QuerySelector {
  ...
  parse(selector) {
    ...
    const onProcess = (
      ...
      tagAll,
      classAll, className,
      pseudoAll, pseudoName, pseudoParam,
      attrAll, attrName, attrOpr, attrVal,
      kinshipAll, kinship
    ) => {
      if (idAll) {
        // id 选择器
