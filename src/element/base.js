...
module.exports = Behavior({
  ...
  attached() {
    ...
    // 根据配置重置全局变量
    const domSubTreeLevel = +config.domSubTreeLevel;
    if (domSubTreeLevel >= 1 && domSubTreeLevel <= MAX_DOM_SUB_TREE_LEVEL) DOM_SUB_TREE_LEVEL = domSubTreeLevel;

    isOriginalSetData = config.setDataMode === 'original';

    // 记录 dom
    this.domNode = cache[pageId].nodeIdMap[nodeId];
    if (!this.domNode) return;

    // 存储 document
    this.document = cache[pageId].document;
  ...
  methods: {
    ...
    callEvent(eventName, evt, extra) {
      ...
      EventTarget.$$process(domNode, eventName, evt, extra, (domNode, evt, isCapture) => {
        ...
        setTimeout(() => {
          ...
          const { window } = cache[this.pageId];
          ...
          if (domNode.tagName === 'A' && evt.type === 'click' && !isCapture) {
            ...
            if (targetDomNode.tagName === 'INPUT') {
              ...
              const { type } = targetDomNode;
              if (type === 'radio') {
                targetDomNode.setAttribute('checked', true);
                const { name } = targetDomNode;
    ...
    getDomNodeFromEvt(evt) {
      ...
      return cache[pageId].nodeIdMap[originNodeId];
    },
