...
function filterNodes(domNode, level, component) {
  const { window } = cache[domNode.$$pageId];
  ...
  return childNodes.map((child) => {
        ...
        if (properties && properties.length) {
          properties.forEach(({ name, get, canBeUserChanged = false }) => {
            ...
            if (canBeUserChanged) {
              ...
              if (oldValues && !isEqual(newValue, oldValues[name], true)) {
                console.warn(
                  '%cQuickBone',
                  'color:#fff;font-weight:bold;background:#f28c06;padding:2px 5px',
                  `检测到小程序原生组件 ${child.behavior} 的属性 ${name} 由 ${oldValues[name]} 变为 ${newValue}，由于 Kbone 的实现问题，该变化将会触发组件属性（extra）的全量更新，请注意该组件是否发生闪动等性能问题，并尽可能避免不必要的自定义组件属性更新，尽可能在用户手势操作结束时再触发。本次更新的全量 patch：`,
                  domInfo.extra,
                );
                domInfo.extra.forceUpdate = true; // 避免被 diff 掉，需要强制更新
              }
            }
          });
        }
    ...
    templateName = !tool.isTagNameSupport(templateName) ? 'not-support' : templateName;
    ...
    for (const { name, get, canBeUserChanged = false } of attrs) {
      ...
      if (canBeUserChanged) {
        ...
        if (!oldData || !isEqual(newValue, oldData[name]) || isOldValuesChanged) {
          ...
          if (isOldValuesChanged) {
            console.warn(
              '%c@tencent/docs-kbone-element',
              'color:#fff;font-weight:bold;background:#1d4eee;padding:2px 5px',
              `检测到小程序原生组件 ${domNode.behavior} 的属性 ${name} 由 ${oldValues[name]} 变为 ${newValue}，由于 Kbone 的实现问题，该变化将会触发组件属性（extra）的全量更新，请注意该组件是否发生闪动等性能问题，并尽可能避免不必要的自定义组件属性更新，尽可能在用户手势操作结束时再触发。本次更新的全量 patch：`,
              destData,
            );
