...
class Element extends Node {
  ...
  $_updateChildrenExtra(node, isRemove) {
    ...
    // 更新 nodeId - dom 映射表
    if (isRemove) {
      cache[this.$_pageId].nodeIdMap[node.$$nodeId] = null;
    } else {
      cache[this.$_pageId].nodeIdMap[node.$$nodeId] = node;
    }
  ...
  $$getBoundingClientRect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.tagName === 'BODY') {
        window.$$createSelectorQuery().selectViewport()
          .scrollOffset(res => (res ? resolve(res) : reject(new Error('body not found in webview'))))
          .exec();
      } else {
        window.$$createSelectorQuery().select(`.miniprogram-root >>> .node-${this.$_nodeId}`)
  ...
  $$getContext() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.tagName === 'CANVAS') {
        ...
        wx.createSelectorQuery().in(this.$$wxComponent)
  ...
  $$getNodesRef() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.tagName === 'CANVAS') {
        // TODO，为了兼容基础库的一个 bug，暂且如此实现
        resolve(wx.createSelectorQuery().in(this.$$wxComponent)
          .select(`.node-${this.$_nodeId}`));
      } else {
  ...
  /**
   * kbone 由于无法实时获取元素的位置大小，没有实现 Element 的位置大小 API，但 html 和 body 这两个元素的位置大小经常会用到而且比较容易模拟
   * 这里对它们进行补全，来支持 flexible 中设置文档文字大小、以及兼容业务侧的各种判断等
   *
   * 这里模拟的是一个 html、body 都固定了宽度高度为一屏，但内容可能发生溢出导致全局滚动的情况
   * 在这种情况下，假设只有 document.documentElement.scrollTop 会变化，其它都是固定的
   */
  get clientWidth() {
    const { window } = cache[this.$_pageId];
    if (window && (this === this.ownerDocument.documentElement || this === this.ownerDocument.body)) {
